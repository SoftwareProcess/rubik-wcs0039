...
class SolveTest(unittest.TestCase):
    ...
    def test_SolveH072_ShouldSolveRightBackMiddleEdgeIncorrectlyInBackLeftMiddlePosition(self):
        parms = {'op':'solve',
                'cube':'obbbbbbbbygrrryrrryrgogrgggyoggooooorybgyyyyowwwwwwwww'}
        result = solve._solve(parms)
        self.assertIn('status', result)
        status = result.get('status', None)
        self.assertEqual(status, 'ok')
        ...
        assertCube = rubik.Cube()
        assertCube.convertString(assertString)
        self.assertTrue(assertCube.right[1][2] == assertCube.right[1][1] and assertCube.back[1][0] == assertCube.back[1][1])
        
    def test_SolveH072_ShouldSolveBackLeftMiddleEdgeIncorrectlyInLeftFrontMiddlePosition(self):
        parms = {'op':'solve',
                'cube':'yyyobbbbbooorrrrrryyyggbgggbggyogooorrgyybrobwwwwwwwww'}
        result = solve._solve(parms)
        self.assertIn('status', result)
        status = result.get('status', None)
        self.assertEqual(status, 'ok')
        ...
        assertCube = rubik.Cube()
        assertCube.convertString(assertString)
        self.assertTrue(assertCube.back[1][2] == assertCube.back[1][1] and assertCube.left[1][0] == assertCube.left[1][1])
        
    def test_SolveY016_ShouldSolveFirstTwoLayersOfScrambledCube(self):
        parms = {'op':'solve',
                'cube':'ogbwybobowrrygbbyyywyrwrbywrwwgboroyggbbowgyrgowgrrgoo'}
        result = solve._solve(parms)
        self.assertIn('status', result)
        status = result.get('status', None)
        self.assertEqual(status, 'ok')
    ...
    def test_SolveY017_ShouldSolveFirstTwoLayersOfSecondScrambledCube(self):
        parms = {'op':'solve',
                'cube':'grbygwywgwwgbrgyywrogybbowoygoooryybrgwrwgwbrroobyobrb'}
        result = solve._solve(parms)
        self.assertIn('status', result)
        status = result.get('status', None)
        self.assertEqual(status, 'ok')
